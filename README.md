...
For a remote MCP client, run streamable HTTP with a public HTTPS URL and OAuth:

```bash
pinner mcp --http --tunnel ngrok \
...
For ChatGPT's OpenAI Secure MCP Tunnel, use an OpenAI-managed tunnel ID and the official `tunnel-client` binary:
