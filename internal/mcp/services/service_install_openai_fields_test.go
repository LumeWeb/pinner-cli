...
// TunnelID and ApiKey are resolved by fieldcraft.Gather (precedence 1) without
...
func TestOpenAIFieldGatherPrepopulatesFromFlags(t *testing.T) {
	...
	fields := append([]fieldcraft.Field[*ServiceInstallState, string]{}, openAIFields()...)
...
func TestOpenAIFieldGatherInvalidIDHardErrors(t *testing.T) {
	...
	src := newServiceInstallValueSource(cmd, "")
	_, _, err := fieldcraft.Gather(context.Background(), src, state, openAIFields())
