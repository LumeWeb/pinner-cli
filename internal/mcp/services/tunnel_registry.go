...
type TunnelProviderSpec struct {
	...
	// prompts. cfgMgr is passed so a field's Derived hook can consult the
	// provider's last-resort credential store (e.g. an ngrok token persisted
	// by a prior Finalize). The step uses Fields+Finalize when set, and falls
	// back to Configurer (legacy) otherwise, so providers migrate one at a time.
	// Nil means the provider has no promptable fields.
	Fields func(s *ServiceInstallState, cfgMgr config.Manager) []fieldform.Field[*ServiceInstallState, string]
