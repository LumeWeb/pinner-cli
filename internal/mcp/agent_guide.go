...
func buildAgentGuide(profile *hostenv.PlatformProfile) AgentGuide {
	...
	spec := toolforge.Guide().
		...
		Flow(toolforge.Flow("vault_share", "Share from a vault").
			...
			Detail(toolforge.Static("Ensure the vault is unlocked (vault_status), then call vault_share with the vault_path to generate a shareable link (control its lifetime with expiry). Only durable (status: durable) files can be shared: if vault_share returns {code:'not_durable', ...}, run vault_flush (non-blocking, returns an accepted job { job_id, profile, path? }), poll vault_flush_status(job_id) or vault_stat until status: durable, then share again. If a file stays non-durable across polls, read vault_stat's flush_attempts and flush_error: a flushing file shows a rising flush_attempts with no error, a failed file shows flush_attempts plus a non-empty flush_error, and a staged file that never started shows zero attempts/no error. The recipient accepts the share with vault_share_accept (accept_state 'pinned' — a pin of the same object key, NOT a digest failure), which is directly visible on tools/list. For multi-profile swarms, list profiles with vault_profiles and hand off a file with vault_send (or pass profile=<name> when more than one profile is unlocked — vault ops return profile_required otherwise). Verify the shared content with vault_verify after the exchange."))).
