...
func NewAgentGuideDescriptor() model.ToolDescriptor {
	guide := AgentGuide{
		Summary: "Start here. Drive Pinner through these primary flows; each step is a tool. Check the current state first, then follow the matching flow. Once a wizard session is active, stay in it: always call the returned next_step_schema via the wizard step tool — do not abandon the wizard to rediscover low-level tools. A static website ZIP (index.html, CSS, JS, images, nested pages) is always a single directory DAG: call upload_file with a host file argument IF capabilities' file_input_policy is host_file_first (your client can hand Pinner a {download_url, file_id} object), otherwise a convert-capable transport source, then publish the resulting directory CID. Do NOT mint a presigned curl URL for a file your host already holds unless capabilities directs you to a transport-scoped source. For guided, interactive website onboarding (human-in-the-loop, step-by-step DNS setup), use the website-onboarding prompt and the websites_wizard tools instead of the publish_website flow.",
		// Rules holds operational invariants the agent must not violate.
		Rules: []string{
		...
		Flows: []GuideFlow{
			...
			},
			{
				...
				Detail: "Check capabilities to pick the byte source THIS client is told to use. If capabilities' file_input_policy is host_file_first (only when your client can hand Pinner a {download_url, file_id} file object), pass a host-provided file reference directly. Otherwise use a transport-scoped source: path in co-located stdio, mint (a presigned HTTP PUT you curl) in HTTP/tunnel mode, or url/data on the OpenAI tunnel. The returned CID is already pinned — use it directly in websites_create/update; do NOT call pins_add after an upload. When using source.mode=mint, poll upload_status with the returned upload_handle for the CID. Static site bundle rule: a ZIP containing index.html, CSS, JS, images, or nested pages is a single directory DAG — call upload_file with the host file argument (or a convert source) and archive_mode=convert, not individual assets.",
			},
			{
				Name:   "vault_upload",
				Title:  "Store a file in a vault",
				Steps:  []string{"capabilities", "vault_put_file"},
				Detail: "Check capabilities to pick the byte source THIS client is told to use. If capabilities' file_input_policy is host_file_first (only when your client can hand Pinner a {download_url, file_id} file object), pass a host-provided file reference directly. Otherwise use a transport-scoped source (host path in co-located stdio mode, a minted presigned PUT in remote mode, or url/data on the OpenAI tunnel) plus the destination vault_path. When using source.mode=mint, poll upload_status with the returned upload_handle for the CID.",
			},
			{
				Name:   "download",
