...
func buildAgentGuide(profile *hostenv.PlatformProfile) AgentGuide {
	...
	spec := toolforge.Guide().
		...
		Flow(toolforge.Flow("vault_share", "Share from a vault").
			...
			Detail(toolforge.Static("Ensure the vault is unlocked (vault_status), then call vault_share with the vault_path to generate a shareable link (control its lifetime with expiry). Only durable (status: ok) files can be shared: if vault_share reports the file is pending, run vault_flush (non-blocking, returns accepted immediately), poll vault_stat until status: ok, then share again. The recipient accepts the share with vault_share_accept, which is directly visible on tools/list. Verify the shared content with vault_verify after the exchange."))).
