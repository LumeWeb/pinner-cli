	...
	return toolforge.Decision("Where are the bytes for the vault?",
		...
		toolforge.Branch("agent-local bytes the host runtime cannot provide through `file` (not a host/user/assistant-generated file)").
			...
			Detail(toolforge.Static("vault_put_file with source.mode=mint + vault_path mints a one-time presigned PUT url bound to vault_path; it has not stored bytes yet.").
				...
				StaticSentence("The vault write is non-blocking: the PUT returns after staging the bytes locally (status: pending); the file is immediately readable from this instance, and durability on Sia happens in the background or via the vault_flush tool (non-blocking, returns accepted immediately) — poll vault_stat until status: ok when durability is needed before sharing. There is no upload_status to poll.")),
...
func buildAgentGuide(profile *hostenv.PlatformProfile) AgentGuide {
	...
	spec := toolforge.Guide().
		...
		Flow(toolforge.Flow("vault_share", "Share from a vault").
			...
			Detail(toolforge.Static("Ensure the vault is unlocked (vault_status), then call vault_share with the vault_path to generate a shareable link (control its lifetime with expiry). Only durable (status: ok) files can be shared: if vault_share reports the file is pending, run vault_flush (non-blocking, returns accepted immediately), poll vault_stat until status: ok, then share again. If a file stays pending across polls, read vault_stat's flush_attempts and flush_error: a rising flush_attempts with a non-empty flush_error means the flush is failing (surface it to the user), while no error means it is still uploading. The recipient accepts the share with vault_share_accept, which is directly visible on tools/list. Verify the shared content with vault_verify after the exchange."))).
