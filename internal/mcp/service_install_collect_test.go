...
func TestServiceInstallStepsShape(t *testing.T) {
	...
	// HEADLESS run: the provider step early-returns inside Execute when
	// non-interactive. Executing with a seeded provider must return immediately
	// without touching the interactive prompt (which would block/fail in a
	// non-TTY test), proving the seeded-path is taken. (Interactive re-runs DO
	// re-prompt with the current provider as an editable default, so the
	// operator can change it.)
	prevNonInteractive := wizard.NonInteractive
	wizard.NonInteractive = true
	defer func() { wizard.NonInteractive = prevNonInteractive }()
