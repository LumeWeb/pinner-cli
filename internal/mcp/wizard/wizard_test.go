...
func TestWebsitesWizard_PlatformSubdomainFlow(t *testing.T) {
	...
	// An empty wizard platform namespace is omitted so the backend applies its
	// authoritative default (the root's own namespace, which may be hns).
	assert.Nil(t, websitesSvc.createCallReq.PlatformNamespace)
