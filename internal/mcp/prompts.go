	...
	return []model.PromptDescriptor{
		{
			...
			Description: "Guides the agent through the website creation wizard workflow step by step using the websites_wizard_start and websites_wizard_step tools. Covers binding a plain domain OR claiming a free platform subdomain (auto-generated by default, or an explicit label when the user requests one) under a platform root. Embeds references to pinner:// resources for platform domains, DNS requirements and validation status. Optional arguments pre-fill wizard choices.",
			Arguments: []model.PromptArgumentDescriptor{
				{Name: ArgDomain, Description: `Domain name for the website (e.g. example.com). If omitted, the website uses a platform (free) subdomain auto-generated by default. Only supply a label when the user explicitly requests one. Do not invent a domain.`},
