...
func TestStampDirectToolsStampsDirectVisible(t *testing.T) {
...
func TestRegisterOfficialDirectToolsRegistersOnlyDirectVisible(t *testing.T) {
	catalog := NewToolCatalog()
	...
	require.NoError(t, RegisterOfficialDirectTools(server, catalog))
}

// TestPlatformDomainAvailabilityNotDirect ensures the read-only, agent-safe
...
func TestPlatformDomainAvailabilityNotDirect(t *testing.T) {
	for _, name := range directToolNamesFor(FullDomainScope) {
...
// TestPlatformDomainsListNotDirect ensures the read-only, agent-safe
// websites_platform_domains_list op is likewise search-only, not on the direct
// tools/list surface.
func TestPlatformDomainsListNotDirect(t *testing.T) {
	for _, name := range directToolNamesFor(FullDomainScope) {
...
// TestStampDirectToolsDoesNotPromoteWizardTools guards the surface-tightening change:
...
func TestStampDirectToolsDoesNotPromoteWizardTools(t *testing.T) {
	catalog := NewToolCatalog()
	...
	for _, name := range []string{
		...
		entry, ok := catalog.Get(name)
		require.True(t, ok, name)
		require.False(t, entry.DirectVisible, "wizard tool %s must remain search-only after stampDirectTools", name)
	}
	// The curated entry points that ARE promoted.
	for _, name := range []string{"auth_status", "vault_create", "vault_status"} {
		entry, ok := catalog.Get(name)
		require.True(t, ok, name)
		require.True(t, entry.DirectVisible, "entry %s must be directly visible after stampDirectTools", name)
	}
}
