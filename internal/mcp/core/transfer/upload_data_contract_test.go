...
// TestDataURIUploadDescComposesUploadCompletionContract pins the LOW DRY
// regression: upload_data's static preamble composes the shared mintcontract
...
// terminal status) — never paraphrases that drop the completed semantics.
func TestDataURIUploadDescComposesUploadCompletionContract(t *testing.T) {
	dataCapable, ok := toolforge.ResolveDescription(transfer.DataURIUploadTargets, hostenv.ProfileGrokHTTP)
	require.True(t, ok, "Grok declares FeatSourceData so the positive copy resolves")
	require.Contains(t, dataCapable,
	...
	relayLess, ok := toolforge.ResolveDescription(transfer.DataURIUploadTargets, hostenv.ProfileHTTPGeneric)
	require.True(t, ok)
