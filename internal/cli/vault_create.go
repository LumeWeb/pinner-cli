...
// staleSeedWarningAfter is how old a pending recovery seed must be before a
// `vault create` warns that it has lingered. It is purely a WARNING
	...
	return &cli.Command{
		...
		Action: func(ctx context.Context, c *cli.Command) error {
			...
			if err != nil {
				...
				// prompt. Only prompt when no profile was provided.
				if c.String(FlagProfile) != "" {
					return err
				}
				// No --profile could be resolved (no default, no active
				// profile): prompt for a name. Use promptui (matching the auth
				// flow) so the prompt renders inline and reads cleanly instead
				// of the raw input echoing on its own line.
			...
			// Guard against overwriting a prior pending seed: a pending seed
			// file (e.g. a prior create that generated a seed but was never
			// restored) is the user's only path back into that vault, and
			// the seed write below runs for every create; overwriting it
			...
			output.Printfln("")
			// Consume the seed file on successful create: the plaintext
			// master recovery mnemonic was persisted early (to survive a
			// mid-flow failure) but must not linger on disk now that the
			// vault is created and the phrase has been shown to the user.
			// If removal fails, surface it: the plaintext master mnemonic
			// lingering on disk is a security concern the user must act on,
			// even though the vault itself is already created.
