...
// via fieldcraft.WithPrompter is visible to steps through fieldcraft.PrompterFrom — this is the
	...
	steps := []Step[*string]{
		StepFunc[*string]{
			...
			ExecuteFunc: func(ctx context.Context, _ *string) error {
				...
				_, _, err := fieldcraft.PrompterFrom(ctx).Select("pick", []string{"x"}, "")
				return err
			},
		},
		StepFunc[*string]{
...
// TestPrompterFromNil verifies fieldcraft.PrompterFrom returns nil (not a panic) when no
// prompter was bound, so step authors can guard cleanly.
func TestPrompterFromNil(t *testing.T) {
	require.Nil(t, fieldcraft.PrompterFrom(context.Background()))
}
...
// step calling fieldcraft.PrompterFrom(ctx) must get the production pterm channel (never a
...
func TestRun_AutoBindsDefaultPrompter(t *testing.T) {
	...
	steps := []Step[*string]{
		StepFunc[*string]{
			...
			ExecuteFunc: func(ctx context.Context, _ *string) error {
				...
				// fieldcraft.NonInteractive returns an "interactive" error, proving both
	...
	s := ""
	// Deliberately no fieldcraft.WithPrompter: Run must bind the default.
