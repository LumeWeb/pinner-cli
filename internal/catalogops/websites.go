...
func websitesValidate(d WebsitesDeps) catalog.Operation {
	return catalog.NewOperation(catalog.OperationSpec{
		...
		MCPTargets: catalog.MCPTargets(
			catalog.Fallback("Validates that a website's DNS records are correctly configured (TXT validation token + _dnslink). Call this after websites_create to confirm DNS propagation. For managed-DNS platform subdomains, validation typically passes within 30-60s of creation; if it fails, wait and retry rather than treating it as a creation failure. For self-managed DNS, ensure the _dnslink TXT and validation TXT are published before calling."),
		),
		Category:    "core",
		Safety:      catalog.SafetyRead,
		Interaction: catalog.InteractionAgentSafe,
