...
func websitesCreate(d WebsitesDeps) catalog.Operation {
	return catalog.NewOperation(catalog.OperationSpec{
		...
		Description: "Create a website that serves an IPFS CID under a custom domain. Provide domain (website) and cid (required), plus optional target-type (ipfs|ipns) and dns-hosting. For newly uploaded content, use the CID returned directly by an upload tool — do NOT call pins_add after upload. pins_add is only needed when the CID originated outside Pinner and must be imported from IPFS. Returns the created website (numeric ID, validation TXT token, DNS records to publish).",
		Category:    "core",
