...
func websitesUpdate(d WebsitesDeps) catalog.Operation {
	return catalog.NewOperation(catalog.OperationSpec{
		...
		Description: "Update an existing website: change its cid, target-type (ipfs|ipns), rename its domain via rename-to, or set dns-hosting (true = Pinner-managed, false = self-managed, omit = unchanged). Selects the site by the website field, then applies whichever optional fields are set (at least one is required). Returns the updated website object. If the target CID was just uploaded (upload_data/upload_file/upload_url with wait=true), a CID_NOT_PINNED failure right after upload usually means the gateway is still propagating the pin — retry the update after a few seconds rather than re-pinning. If the CID was never uploaded through Pinner, pin it first (pins_add with wait=true). When only cid is given (no target-type), the site's current target type is preserved automatically, so you do not need to look it up first. For the full guided flow, fetch the website-update prompt (prompts/get website-update).",
		Category:    "core",
		Safety:      catalog.SafetyMutate,
		Interaction: catalog.InteractionAgentSafe,
		Visibility:  catalog.VisibilityBoth,
		...
		Args: []catalog.OperationArg{
			...
			{Name: "cid", Type: catalog.ArgTypeString, Help: "New target CID", AgentHelp: "The IPFS CID to serve. If you just uploaded this CID (upload_data/upload_file/upload_url with wait=true), a CID_NOT_PINNED failure right after upload usually means the gateway is still propagating the pin — retry the update after a few seconds rather than re-pinning. Only if the CID was never uploaded through Pinner should you first run pins_add(cids=[\"<cid>\"], wait=true) and confirm it succeeds BEFORE calling this update. When a bare cid is set without target-type, the site's current IPFS/IPNS targeting is preserved automatically."},
