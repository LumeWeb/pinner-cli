...
var websitesCreateDesc = toolforge.Static(
	...
	Static("The CID must be a directory whose root contains index.html — gateways serve /index.html at the directory root.").
	...
	Static("Do NOT combine a multi-file website into a single HTML file before publishing.").
	...
	Static("For a single HTML file, use upload_file with wrap=true and do NOT set an explicit name — the tool auto-names wrapped HTML to index.html so the site resolves at root.").
	...
	Static("For newly uploaded content, use the CID returned by the upload tool directly — do NOT call pins_add after upload.").
	Static("pins_add is only needed when the CID originated outside Pinner and must be imported from IPFS.").
