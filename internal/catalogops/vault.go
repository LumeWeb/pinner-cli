...
func vaultStat(d VaultDeps) catalog.Operation {
	return catalog.NewOperation(catalog.OperationSpec{
		...
		MCPTargets: catalog.MCPTargets(
			catalog.Fallback("Show metadata for a single vault path: type, size, media type, content digest, object ID, and current status (ok | pending | lost). Returns metadata only, never the content. While a file is staged (status is pending or lost) the result also carries flush_attempts and flush_error: a rising flush_attempts count with a non-empty flush_error means the durability flush is failing (surface the error to the user), while zero attempts / no error means it is still queued or uploading. Both fields are omitted once the file is durable (status is ok)."),
		),
		Category:    "vault",
		Safety:      catalog.SafetyRead,
		Interaction: catalog.InteractionAgentSafe,
